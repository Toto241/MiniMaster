...
| Static readiness checks | scripts/static_readiness_checks.py: 26/26 checks passed (100%) (python scripts/test_automation.py --suite static-readiness) | ✅ | Automated | 2026-04-22 |
| CodeQL security scan (0 high/critical) | ⛔ Blocked by GitHub Actions billing; local security test suites pass | ⛔ | Engineering | 2026-04-22 |
| Android build (if applicable) | ⛔ Blocked by GitHub Actions billing; local static readiness checks pass | ⛔ | Engineering | 2026-04-22 |
| Deployment result | Lokale Deploy-Bereitschaft am 2026-04-22 geprüft: Firebase CLI, Projektbindung und Login sind vorhanden, aber belastbarer Final-Deploy aus diesem Workspace nicht ausgeführt, weil produktive Runtime-Secrets/-Config lokal nicht vorliegen (`.env` / `.runtimeconfig.json` fehlen) | ⛔ | Engineering | 2026-04-22 |

### 3.2 Functional Commissioning Gate

| Checklist Key | Result | Evidence Link | Tester | Date |
| --------------- | -------- | --------------- | -------- | ------ |
| android-apps (pairing + sync) | ⬜ | build/test-automation/latest-summary.json shows the Android device suites still skipped because no connected Android device or emulator is visible via adb; local validation on 2026-04-22 additionally confirmed that this Windows host has no AVD profile and no local avdmanager/sdkmanager to create the required single emulator yet | Automated + QA/Operations pending | 2026-04-22 |
...
| Secrets/config review | Repo-seitige Bereinigung abgeschlossen; SRI fuer app-check-compat in admin-panel ergaenzt; verbleibend ist die externe Firebase-Key-Rotation inkl. Restriktionsnachweis aus der Console | ⬜ | Security Owner | 2026-04-22 |
...
| On-call / escalation roster | [ONCALL_ESCALATION_ROSTER.md](ONCALL_ESCALATION_ROSTER.md) ist operationalisiert; reale Namen, Kontakte, Reachability und Sign-off noch offen | ⬜ | Operations Lead | 2026-04-22 |
...
| GitHub Actions Billing/Spending-Limit bereinigen | Aktuelle Revalidation zeigt fuer CodeQL und Android CI `blocked / billing`; lokale Tests sind grün | ⬜ | Repo Owner | offen |
| CodeQL-Ergebnis verlinken | Frischer Nachweis pending (Billing-Blocker); lokale Security-Tests grün | ⬜ | Engineering Owner | offen |
| Android CI Build-Nachweis verlinken | Frischer Nachweis pending (Billing-Blocker); lokale static-readiness grün | ⬜ | Engineering Owner | offen |
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | CodeQL und Android CI nach Repo-Fix neu ausfuehren | Engineering Owner | kurzfristig | GitHub Actions Billing | Aktualisierte [CI_REVALIDATION_LATEST.md](CI_REVALIDATION_LATEST.md) mit aktuellen Runs | ⛔ |
