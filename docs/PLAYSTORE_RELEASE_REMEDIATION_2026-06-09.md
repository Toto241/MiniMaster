...
| 8 | Git/GitHub blocker | Diagnosed/fixed where repo-side | The empty Android Release Bundles failure was a workflow validation error (`runner.temp` in job-level `env`) and is fixed. CI, Android CI, Node CI, CodeQL, and Dependency Submission are green on `e391dab`; the later Gradle classpath hardening passes local Android validation and is being pushed separately. GitHub Code Scanning is still disabled and API activation returns HTTP 403. Dependabot still requires a post-push dependency-graph refresh or advisory review for the remaining `settings.gradle` alerts. |
