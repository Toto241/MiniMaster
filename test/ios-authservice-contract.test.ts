...
describe("ios AuthService contract migration", () => {
  ...
  it("iOS README documents customToken registration flow and no legacy secretKey login", async () => {
    const readme = await readUtf8("iosMasterApp/README.md");
    expect(readme).toContain("registerMasterDevice(imei, deviceName?)");
    expect(readme).toContain("{ masterId, customToken }");
