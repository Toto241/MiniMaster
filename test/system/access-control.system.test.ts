...
describe("system: callable access control", () => {
  ...
  it("erlaubt authentifizierten Zugriff und liefert Subscription-Status", async () => {
    ...
    expect(result).toEqual({ subscriptionStatus: { status: "active", type: "premium" }, hasAccess: true, childLimit: 4, parentAppLimit: 2, expiresDateMs: null, originalTransactionId: null, platform: "unknown" });
  });
});
