...
- [ ] **Subscription Scheduler – Periodische Play-API-Re-Verifikation** (optional Follow-up): Täglicher Zusatz-Scan ruft Play Developer API `purchases.subscriptions.get` für aktive Abos mit gespeichertem `purchaseToken` auf, um stille Klärungen/Refunds außerhalb von RTDN aufzufangen. RTDN-Webhook (primärer Pfad) ist erledigt; siehe Durchgeführt-Sektion.
