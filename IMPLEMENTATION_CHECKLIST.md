...
- [ ] **F6 CSP-Refactor** (in Bearbeitung – Stufe 1 fertig): ~80+ Inline-`onclick`/`style` in `admin-panel/index.html` zu `addEventListener`/CSS-Klassen migrieren. Erfordert Anpassung der DOM-Snapshot-Tests.
  ...
  - [x] **Stufe 3 (Tab-Content Buttons en bloc)**: Regex-basierte Massenmigration aller verbleibenden trivialen Inline-Handler — Pattern 1 `onclick="fn()"` → `data-action="fn"` (skip reserviert `logout`), Pattern 2 `onclick="fn('arg')"` → `data-action="fn" data-args='["arg"]'`. 85 Handler in einem Sweep umgestellt (Tabs: Stats, Users, Devices, Subscriptions, Pairing, Tickets, Logs, GDPR, Auth, Operator-Config, QA-Dashboard, Commissioning, Testing-Register, Python-Automation, QA-Platform, Emulator-Lab, Suite, Self-Healing, Setup-Validation, RBAC, Functions, Knowledge-Base, Audit, FCM, AI, Legal, Play-Store, Auth-Toggles). Inline-`onclick=` 87 → 2 (-85); Restmenge sind 2 Handler mit dynamischen Expression-Args (`copyRolloutBundleScript`/`downloadRolloutBundleScript` lesen `getOperatorConfigFormValues().cloud.projectId || firebaseConfig.projectId` zur Klick-Zeit). Bundle-Budget Test ratchet auf `MAX_INLINE_ONCLICK = 5`. Multiline-`filterTickets`-Block normalisiert. Suite 69/69 ✅, 2149/2149 ✅.
  - [ ] Stufe 4: Inline-`style="…"` (~187) in CSS-Klassen migrieren (sowie 2 Rest-`onclick` mit Wrapper-Funktion entfernen)
