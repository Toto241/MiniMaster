...
- [x] **F6.1 CSS-Hardening**: Alle 187 Inline-`style="…"`-Attribute in [admin-panel/index.html](admin-panel/index.html) durch deterministische Utility-Klassen `mm-u001`–`mm-u051` ersetzt (51 unique Werte). Neue Datei [admin-panel/styles-utilities.css](admin-panel/styles-utilities.css) (5,6 KB) wird via `<link rel="stylesheet">` geladen und im Service-Worker pre-cached. [firebase.json](firebase.json) admin-panel CSP härtet `style-src 'self'` (ohne `'unsafe-inline'`). 3 neue Tests in `admin-panel-bundle-budget.test.ts`: (1) `style="`-Count = 0 + Stylesheet-Link-Verifikation, (2) Integritäts-Test (jede genutzte `mm-uXXX`-Klasse ist in der CSS-Datei definiert), (3) admin-panel CSP `style-src` ohne `'unsafe-inline'`. Damit ist das Admin-Panel vollständig CSP-strikt: `script-src 'self' …` und `style-src 'self'` — beide ohne `'unsafe-*'`. Suite 69/69 ✅, 2154/2154 ✅.
