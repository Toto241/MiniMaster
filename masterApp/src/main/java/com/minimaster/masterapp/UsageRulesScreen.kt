...
fun UsageRulesScreen(
    ...
    Scaffold(
        ...
        UsageRulesContent(
            ...
            onUpdateDailyLimit = viewModel::updateDailyLimit,
            onUpdateAllowedStartTime = viewModel::updateAllowedStartTime,
            onUpdateAllowedEndTime = viewModel::updateAllowedEndTime,
            onAddPerAppLimit = viewModel::addPerAppLimit,
            onRemovePerAppLimit = viewModel::removePerAppLimit,
...
@Composable
fun UsageRulesContent(
    state: UsageRulesState,
    modifier: Modifier = Modifier,
    onUpdateDailyLimit: (Int) -> Unit,
    onUpdateAllowedStartTime: (String) -> Unit,
    onUpdateAllowedEndTime: (String) -> Unit,
    onAddPerAppLimit: (String, Int) -> Unit,
    onRemovePerAppLimit: (String) -> Unit,
    onSaveRules: () -> Unit,
) {
    Column(
        modifier = modifier
        ...
        var dailyLimitText by remember(state.dailyLimitMinutes) {
            mutableStateOf(if (state.dailyLimitMinutes > 0) state.dailyLimitMinutes.toString() else "")
        }
        OutlinedTextField(
        ...
        Row(
            ...
            var startTime by remember(state.allowedStartTime) { mutableStateOf(state.allowedStartTime) }
            var endTime by remember(state.allowedEndTime) { mutableStateOf(state.allowedEndTime) }

            OutlinedTextField(
                value = startTime,
                onValueChange = {
                    startTime = it
                    onUpdateAllowedStartTime(it)
                },
        ...
        state.perAppLimits.forEach { (pkg, minutes) ->
            Row(
                ...
                IconButton(onClick = { onRemovePerAppLimit(pkg) }) {
            ...
            Button(
                onClick = {
                    ...
                    if (newPkg.isNotBlank() && limit != null && limit > 0) {
                        onAddPerAppLimit(newPkg.trim(), limit)
                        newPkg = ""
                        newLimit = ""
                    }
                }
