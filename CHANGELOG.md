...
### Added
- **iOS-Parität (Kind-App) — P0-Lücken geschlossen (Repo-seitig):**
  - **Aufgaben-Foto-Upload (A2):** Die iOS-Kind-App kann jetzt einen Foto-Nachweis zu einer Aufgabe einreichen. Neue `TaskProofView` (PhotosPicker) wird aus `MainChildView` über einen Kamera-Button (nur bei `pending`) als Sheet geöffnet; der Upload nutzt den bestehenden `PhotoProofService` (`iosSharedServices/`) → Firebase Storage `proofs/{childId}/{taskId}/` → `completeTask`-Callable. `FirebaseStorage` zum `MiniMasterChild`-Target ergänzt; Lokalisierung in de/en/es/fr/it; `taskError` ist nun als Alert sichtbar.
  - **Nutzungslimit-Enforcement (A1):** Neue `DeviceActivityMonitor`-Extension (`iosChildApp/DeviceActivityMonitorExtension/`) erzwingt Tageslimits real — `eventDidReachThreshold` setzt einen ManagedSettings-Shield, `intervalDidEnd` hebt ihn zum Tageswechsel auf. `AppBlockingManager.applyUsageRules` registriert dafür ein `DeviceActivityEvent` mit `threshold`. Cross-Prozess-Brücke `SharedPolicyDefaults` (App-Group `group.com.minimaster.childapp`); der Host meldet `usage_limit_reached` beim nächsten Vordergrund über das bestehende `publishDeviceEvent` (kein Backend-Change).
  - 2 neue statische Contract-Tests (`test/ios-task-photo-upload-contract.test.ts`, `test/ios-deviceactivity-monitor-contract.test.ts`, 10 Tests). Hinweis: Extension-Target-Anlage, Xcode-Quelldatei-Mitgliedschaft und realer Geräte-Nachweis bleiben Mac-Schritte (in `iosChildApp/README.md` dokumentiert).
