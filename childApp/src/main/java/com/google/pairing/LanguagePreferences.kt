...
fun applySavedChildLocale(context: Context) {
...
fun hasChildLanguageSelection(context: Context): Boolean {
    val prefs = context.getSharedPreferences(LANGUAGE_PREFS_NAME, Context.MODE_PRIVATE)
    return prefs.getBoolean(KEY_LANGUAGE_SELECTED, false)
}

fun saveChildLanguageSelection(context: Context, languageTag: String) {
    val prefs = context.getSharedPreferences(LANGUAGE_PREFS_NAME, Context.MODE_PRIVATE)
    prefs.edit()
